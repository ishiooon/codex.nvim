-- このファイルはエディタの選択範囲を追跡し、Codexへ送信する処理を実装します。
...
function M.update_selection()
  ...
  -- Codex ターミナル用バッファでは選択状態を更新しない
  if terminal_buffer.is_codex_terminal_buffer(current_buf) then
    -- Codex ターミナルに移動した場合は保留中の降格を解除する
...
---@param value any
---@return any
local function safe_deepcopy(value)
...
---保留中の選択通知があれば送信し、保持している内容を消去する
---@return boolean sent true when a pending selection was sent
function M.flush_pending_selection()
  if not M.state.pending_selection then
    return false
  end

  local codex_main = require("codex")
  if type(codex_main.is_codex_connected) == "function" and not codex_main.is_codex_connected() then
    return false
  end

  M.send_selection_update(M.state.pending_selection)
...
function M.send_current_selection()
  if not M.state.tracking_enabled or not M.server then
    logger.error("selection", "Codex is not running")
    return
  end
...
---@メンションの挿入に合わせてファイル表記を揃えた選択情報を作る
---@param selection table
---@param formatted_path string
---@return table
local function build_selection_for_at_mention(selection, formatted_path)
  if type(selection) ~= "table" then
    return selection
  end

  local aligned = safe_deepcopy(selection)
  aligned.filePath = formatted_path

  if type(formatted_path) == "string" and formatted_path:match("^/") then
    aligned.fileUrl = "file://" .. formatted_path
  else
    aligned.fileUrl = selection.fileUrl or ("file://" .. selection.filePath)
  end
...
---@param selection table
---@return string
local function build_terminal_fallback_payload(selection)
  if type(selection) ~= "table" then
    return ""
  end
  ...
  local body = type(selection.text) == "string" and selection.text or ""
  if body ~= "" then
    return header .. "\n" .. body
  end
  return header
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  -- Check if Codex integration is running (server may or may not have clients)
  local codex_main = require("codex")
  if not codex_main.state.server then
    logger.error("selection", "Codex integration is not running.")
    return false
  end
  ...
  -- Codex側のテキスト挿入に必要なため、送信対象の選択内容を先に通知する
  local is_connected = true
  if type(codex_main.is_codex_connected) == "function" then
    is_connected = codex_main.is_codex_connected()
  end

  -- Codex側で@メンションと選択通知が一致するよう、同じファイル表記に揃える
  local selection_for_insert = sel_to_send
  if type(codex_main._format_path_for_at_mention) == "function" then
    local ok, formatted_path = pcall(codex_main._format_path_for_at_mention, sel_to_send.filePath)
    if ok and type(formatted_path) == "string" and formatted_path ~= "" then
      selection_for_insert = build_selection_for_at_mention(sel_to_send, formatted_path)
    end
  end

  if is_connected then
    M.send_selection_update(selection_for_insert)
  else
    -- 接続確立後に送るため、選択内容を内部状態へ保存する
    M.state.pending_selection = safe_deepcopy(selection_for_insert)

    -- Codex未接続時はターミナルへ本文を流し込み、入力欄を更新する
    local config = codex_main.state and codex_main.state.config or {}
    local allow_fallback = config.fallback_to_terminal_send ~= false
    if allow_fallback then
      local payload = build_terminal_fallback_payload(selection_for_insert)
      if payload ~= "" then
        -- ターミナルへ送信すると入力欄の内容が変更される
        terminal.send(payload, { submit = true })
      end
    end
  end
