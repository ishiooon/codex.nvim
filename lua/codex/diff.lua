-- このファイルはCodexの差分表示と受け入れ・拒否の操作を実装します。
--- Diff module for Codex Neovim integration.
...
---Find the Codex terminal window to keep focus there.
---Uses the terminal provider to get the active terminal buffer, then finds its window.
---@return number? win_id Window ID of the Codex terminal window, or nil if not found
...
---Display existing Codex terminal in new tab
...
local function display_terminal_in_new_tab()
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    desc = "Auto-enter terminal mode when focusing Codex terminal",
  })
...
function M.open_diff_blocking(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local success, err = pcall(M._setup_blocking_diff, {
    ...
    if resume_success then
      ...
      if _G.codex_deferred_responses and _G.codex_deferred_responses[co_key] then
        _G.codex_deferred_responses[co_key](resume_result)
        _G.codex_deferred_responses[co_key] = nil
      else
      ...
      local co_key = tostring(co)
      if _G.codex_deferred_responses and _G.codex_deferred_responses[co_key] then
        _G.codex_deferred_responses[co_key]({
          error = {
            code = -32603,
...
function M.close_diff_by_tab_name(tab_name)
  ...
  if diff_data.status == "saved" then
    -- Codex CLI has written the file, reload any open buffers
