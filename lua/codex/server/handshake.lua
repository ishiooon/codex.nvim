-- このファイルはWebSocketハンドシェイクの検証と応答生成を担当します。
...
---@param auth_mode string|nil Authentication mode (required/optional/disabled)
...
function M.validate_upgrade_request(request, expected_auth_token, auth_mode)
  local headers = utils.parse_http_headers(request)
  -- 認証の厳格さを指定しない場合は既存挙動を維持する
  local normalized_auth_mode = auth_mode or "required"
  ...
  if expected_auth_token and normalized_auth_mode ~= "disabled" then
    ...
    local auth_header = headers["x-codex-code-ide-authorization"]
    if not auth_header then
      if normalized_auth_mode == "optional" then
        return true, headers
      end
      return false, "Missing authentication header: x-codex-code-ide-authorization"
    end
...
function M.process_handshake(request, expected_auth_token, auth_mode)
  ...
  local is_valid_upgrade, validation_payload = M.validate_upgrade_request(request, expected_auth_token, auth_mode) ---@type boolean, table|string
