-- このファイルはWebSocketクライアントの状態管理と送受信処理を担当します。
...
function M.process_data(client, data, on_message, on_close, on_error, auth_token, auth_mode)
  ...
  if not client.handshake_complete then
    ...
    if complete and request then
      ...
      local success, response_from_handshake, headers =
        handshake.process_handshake(request, auth_token, auth_mode)

      -- Log authentication results
      if success then
        if auth_token then
          -- optionalモードでは認証ヘッダー無しの接続を許可する
          if auth_mode == "optional" and headers and not headers["x-codex-code-ide-authorization"] then
            logger.warn("client", "Authentication header missing, accepted due to optional mode:", client.id)
          else
      ...
      client.tcp_handle:write(response_from_handshake, function(err)
        ...
        if success then
          ...
          if #client.buffer > 0 then
            M.process_data(client, "", on_message, on_close, on_error, auth_token, auth_mode)
          end
        else
