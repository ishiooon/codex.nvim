-- このファイルはCodex.nvimのWebSocketサーバ起動とハンドラ登録を担当します。
...
function M.start(config, auth_token)
  ...
  if auth_token then
    logger.debug("server", "Starting WebSocket server with authentication enabled (mode=" .. tostring(config.auth_mode) .. ")")
...
function M._setup_deferred_response(deferred_info)
  ...
  _G.codex_deferred_responses[tostring(co)] = response_sender
