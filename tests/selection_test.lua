...
describe("Range Selection Tests", function()
  ...
  describe("send_at_mention_for_visual_selection with range", function()
    ...
    before_each(function()
      mock_server = {
        ...
        broadcast = function(event, params)
          ...
          table.insert(mock_server.broadcasts, mock_server.last_broadcast)
          return true
        end,
      }
      ...
      mock_codex_main = {
        ...
        _format_path_for_at_mention = function(file_path)
          if file_path == "/test/file.lua" then
        ...
        send_at_mention = function(file_path, start_line, end_line, context)
          ...
          if type(mock_codex_main._format_path_for_at_mention) == "function" then
            formatted_path = mock_codex_main._format_path_for_at_mention(file_path)
          end
    ...
    it("should send range selection successfully", function()
      ...
      assert(result == true)
      -- Codex側の挿入に必要なため、選択範囲の通知が先に送られることを確認する
      assert(#mock_server.broadcasts == 2)
      assert(mock_server.broadcasts[1].event == "selection_changed")
      assert(mock_server.broadcasts[1].params.text == "line 2\nline 3\nline 4")
      assert(mock_server.broadcasts[1].params.filePath == "test/file.lua")
      assert(mock_server.broadcasts[1].params.selection.start.line == 1)
      assert(mock_server.broadcasts[1].params.selection["end"].line == 3)
      assert(mock_server.broadcasts[2].event == "at_mentioned")
      assert(mock_server.broadcasts[2].params.filePath == "test/file.lua")
      assert(mock_server.broadcasts[2].params.lineStart == 1) -- 0-indexed
      assert(mock_server.broadcasts[2].params.lineEnd == 3) -- 0-indexed
    end)
    ...
    it("should fall back to existing logic when no range provided", function()
      ...
      assert(#mock_server.broadcasts == 2)
      assert(mock_server.broadcasts[1].event == "selection_changed")
      assert(mock_server.broadcasts[1].params.text == "tracked text")
      assert(mock_server.broadcasts[2].params.lineStart == 0)
      assert(mock_server.broadcasts[2].params.lineEnd == 0)
    end)
    ...
    it("should queue selection update while disconnected and flush after connect", function()
      -- 接続前は選択通知を保留し、接続後に送信できることを確認する
      mock_codex_main.is_codex_connected = function()
        return false
      end
      mock_codex_main.send_at_mention = function()
        return true, nil
      end
      ...
      assert(result == true)
      assert(#mock_server.broadcasts == 0)

      mock_codex_main.is_codex_connected = function()
        return true
      end

      selection.flush_pending_selection()

      assert(#mock_server.broadcasts == 1)
      assert(mock_server.broadcasts[1].event == "selection_changed")
      assert(mock_server.broadcasts[1].params.text == "line 2\nline 3\nline 4")
    end)
  end)
