...
describe("codex.terminal (wrapper for Snacks.nvim)", function()
  ...
  before_each(function()
    ...
    mock_codex_config_module = {
      apply = spy.new(function(user_conf)
        local base_config = { terminal_cmd = "codex" }
  ...
  describe("terminal.open", function()
    it(
      "should call Snacks.terminal.open with default 'codex' command if terminal_cmd is not set in main config",
      function()
        vim.g.codex_user_config = {}
        mock_codex_config_module.apply = spy.new(function()
          return { terminal_cmd = "codex" }
        end)
        package.loaded["codex.config"] = mock_codex_config_module
        package.loaded["codex.terminal"] = nil
        terminal_wrapper = require("codex.terminal")
        ...
        assert.are.equal("codex", cmd_arg)
        assert.is_table(env_arg)
        assert.are.equal("true", env_arg.ENABLE_IDE_INTEGRATION)
        -- Codex CLI が同じロックファイルを参照できるように設定されることを確認する
        local lockfile = require("codex.lockfile")
        local expected_dir = lockfile.lock_dir:gsub("/+$", ""):gsub("/ide$", "")
        assert.are.equal(expected_dir, env_arg.CODEX_CONFIG_DIR)
    ...
    it("should call Snacks.terminal.open with terminal_cmd from main config", function()
      vim.g.codex_user_config = { terminal_cmd = "my_codex_cli" }
      mock_codex_config_module.apply = spy.new(function()
        return { terminal_cmd = "my_codex_cli" }
      end)
      package.loaded["codex.config"] = mock_codex_config_module
      package.loaded["codex.terminal"] = nil
      terminal_wrapper = require("codex.terminal")
      terminal_wrapper.setup({}, "my_codex_cli")

      terminal_wrapper.open()
      mock_snacks_provider.open:was_called(1)
      local cmd_arg = mock_snacks_provider.open:get_call(1).refs[1]
      assert.are.equal("my_codex_cli", cmd_arg)
    end)
    ...
    it("should restart terminal when Codex port changes", function()
      terminal_wrapper.open()
      mock_snacks_provider.open:was_called(1)
      local port_state = terminal_wrapper._get_port_state_for_test()
      assert.are.equal(12345, port_state.last_sse_port)
      assert.is_true(port_state.has_launched)

      -- 接続ポートが変わった場合は既存ターミナルを閉じて再起動する
      local server_module = require("codex.server.init")
      server_module.state.port = 23456
      assert.are.equal(23456, terminal_wrapper._get_current_sse_port_for_test())

      terminal_wrapper.open()
      mock_snacks_provider.close:was_called(1)
      mock_snacks_provider.open:was_called(2)
    end)
  ...
  describe("terminal.toggle", function()
    it("should call Snacks.terminal.toggle with correct command and options", function()
      vim.g.codex_user_config = { terminal_cmd = "toggle_codex" }
      mock_codex_config_module.apply = spy.new(function()
        return { terminal_cmd = "toggle_codex" }
      end)
      package.loaded["codex.config"] = mock_codex_config_module
      package.loaded["codex.terminal"] = nil
      terminal_wrapper = require("codex.terminal")
      terminal_wrapper.setup({ split_side = "left", split_width_percentage = 0.4 }, "toggle_codex")
      ...
      assert.are.equal("toggle_codex", cmd_arg)
      assert.are.equal("left", config_arg.split_side)
  ...
  describe("command arguments support", function()
    it("should append cmd_args to base command when provided to open", function()
      ...
      mock_snacks_provider.open:was_called(1)
      local cmd_arg = mock_snacks_provider.open:get_call(1).refs[1]
      assert.are.equal("codex --resume", cmd_arg)
    end)
    ...
    it("should append cmd_args to base command when provided to toggle", function()
      ...
      mock_snacks_provider.simple_toggle:was_called(1)
      local cmd_arg = mock_snacks_provider.simple_toggle:get_call(1).refs[1]
      assert.are.equal("codex --resume --verbose", cmd_arg)
    end)

    it("should work with custom terminal_cmd and arguments", function()
      terminal_wrapper.setup({}, "my_codex_binary")
      terminal_wrapper.open({}, "--flag")

      mock_snacks_provider.open:was_called(1)
      local cmd_arg = mock_snacks_provider.open:get_call(1).refs[1]
      assert.are.equal("my_codex_binary --flag", cmd_arg)
    end)
    ...
    it("should handle special characters in arguments", function()
      ...
      mock_snacks_provider.open:was_called(1)
      local cmd_arg = mock_snacks_provider.open:get_call(1).refs[1]
      assert.are.equal("codex --message='hello world'", cmd_arg)
    end)
    ...
    it("should maintain backward compatibility when no cmd_args provided", function()
      ...
      assert.are.equal("codex", open_cmd)
      ...
      assert.are.equal("codex", toggle_cmd)
    end)
  end)
